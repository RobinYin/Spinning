...
{
    _snsTableView.frame = CGRectMake(_snsTableView.frame.origin.x, _snsTableView.frame.origin.y + StatusHeaderHight + StatusBarHeight, _snsTableView.frame.size.width, 220);
    [_snsTableView setSeparatorColor:[UIColor clearColor]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([UMSocialAccountManager isOauthWithPlatform:snsPlatform.platformName]) {
        ...
        showUserName = [NSString stringWithFormat:@"尚未绑定"];
    }
    
    if ([showUserName isEqualToString:@""]) {
        cell.textLabel.text = @"已绑定";
    }
    else{
