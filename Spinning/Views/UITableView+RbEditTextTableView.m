//
//  UITableView+RbEditTextTableView.m
//  Spinning
//
...
#define kNotificationObjectsKey		@"TextFieldItAdditionsNotificationObservers"

@implementation UITableView (RbEditTextTableView)
												 ...
												 toIndexPath: (NSIndexPath *) toIndexPath
{
	...
	for( NSInteger section=[fromIndexPath section]; section<=[toIndexPath section] && !found; section++ )
	{
		...
		for( NSInteger row=startRow; row<=endRow && !found; row++ )
		{
			...
			if( !([currentFirstResponderIndexPath section] == section && [currentFirstResponderIndexPath row] == row) )
			{
				...
				{
					aNewIndexPath = currentIndexPath;
					found = YES;
				}
			}
		}
	...
	if( aNewIndexPath )
	{
		[self scrollToRowAtIndexPath: aNewIndexPath atScrollPosition: UITableViewScrollPositionMiddle animated: YES];
	}
...
-(NSMutableArray *) notificationObservers
{
	...
	{
		notificationObservers = [[NSMutableArray new]autorelease];
